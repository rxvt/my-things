"""Tests for the Lister Textual app — list index screen."""
...
@pytest.fixture
def app() -> ListerApp:
    """Return a ListerApp instance backed by an in-memory database."""
    return ListerApp(db_path=_MEMORY)


async def test_app_has_footer(app: ListerApp) -> None:
...
async def test_app_has_list_view(app: ListerApp) -> None:
    """The app should render a ListView widget."""
    async with app.run_test() as pilot:
        list_view = pilot.app.query(ListView)
        assert len(list_view) == 1


async def test_list_view_contains_games(app: ListerApp) -> None:
...
async def test_list_view_item_count(app: ListerApp) -> None:
...
async def test_first_item_is_highlighted(app: ListerApp) -> None:
